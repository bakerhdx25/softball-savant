...
PIPELINE_SUMMARY = OUTPUT_ROOT / "official_pipeline_summary.json"


def current_snapshot_id() -> str:
    override = os.environ.get("SOFTBALL_SAVANT_SNAPSHOT")
    if override:
        return override
    if PIPELINE_SUMMARY.is_file():
        return json.loads(PIPELINE_SUMMARY.read_text(encoding="utf-8"))["snapshot_id"]
    return "20260704T165430Z"
...
SITE_DATA_DESTINATION = SITE_ROOT / "data" / "site-data.json"
...
def pythagorean_study(periods: dict[str, dict[str, Any]]) -> dict[str, Any]:
    rows = []
    for season in ("2025", "2026"):
        for team in periods[season]["teams"]:
            runs = number(team["summary"].get("runs"))
            allowed = number(team["summary"].get("runsAllowed"))
            decisions = number(team["record"].get("wins")) + number(team["record"].get("losses"))
            if runs and allowed and decisions:
                rows.append({"season": season, "runs": runs, "allowed": allowed, "winPct": number(team["record"]["wins"]) / decisions})

    def mse(sample: list[dict[str, Any]], exponent: float) -> float:
        errors = []
        for row in sample:
            expected = row["runs"] ** exponent / (row["runs"] ** exponent + row["allowed"] ** exponent)
            errors.append((expected - row["winPct"]) ** 2)
        return sum(errors) / len(errors)

    candidates = [value / 100 for value in range(50, 351)]
    fitted = min(candidates, key=lambda exponent: mse(rows, exponent))
    cross_validation = []
    for training_season in ("2025", "2026"):
        training = [row for row in rows if row["season"] == training_season]
        testing = [row for row in rows if row["season"] != training_season]
        exponent = min(candidates, key=lambda value: mse(training, value))
        cross_validation.append({
            "trainingSeason": int(training_season), "fittedExponent": exponent,
            "testMSE": mse(testing, exponent), "standardTestMSE": mse(testing, 2.0),
        })
    return {
...
def build_period(period: str, base_teams: list[dict[str, Any]] | None = None, write: bool = True) -> dict[str, Any]:
    ...
    payload = {
        "meta": {
            ...
            "sourceNote": "Official AUSL public schedule, play-by-play, box score, and stats data.",
        },
...
def build() -> dict[str, Any]:
    periods = {
        "2026": build_period("2026", write=False),
        "2025": build_period("2025", write=False),
        "combined": build_period("2025-2026", write=False),
    }

    tto_path = WAR_ROOT / "scripts" / "build-tto-site.py"
    spec = importlib.util.spec_from_file_location("ausl_tto_site", tto_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load TTO builder from {tto_path}")
    tto_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(tto_module)
    tto = tto_module.build_payload(load(PA_SOURCE))

    leaderboards: dict[str, list[dict[str, Any]]] = {}
    for season in (2025, 2026):
        rows = load(OUTPUT_ROOT / f"combined_{season}.json")
        position_rows = {row["player_key"]: row for row in load(OUTPUT_ROOT / f"position_players_{season}.json")}
        headshots = load(OUTPUT_ROOT / f"headshots_{season}.json")
        for row in rows:
            position = position_rows.get(row["player_key"], {})
            for field in ("wOBA", "H", "HR", "BB", "SO"):
                row[field] = position.get(field)
            entry = headshots.get(row["player_key"], {})
            local_path = entry.get("local_path")
            if local_path:
                source_headshot = OUTPUT_ROOT / local_path
                destination_headshot = HEADSHOT_DESTINATION / Path(local_path).name
                if source_headshot.is_file():
                    HEADSHOT_DESTINATION.mkdir(parents=True, exist_ok=True)
                    shutil.copy2(source_headshot, destination_headshot)
                    row["headshot"] = f"assets/headshots/{destination_headshot.name}"
                else:
                    row["headshot"] = None
            else:
                row["headshot"] = None
        leaderboards[str(season)] = rows

    for payload in periods.values():
        for team in payload["teams"]:
            runs = number(team["summary"].get("runs"))
            allowed = number(team["summary"].get("runsAllowed"))
            games = number(team.get("games"))
            denominator = runs ** 2 + allowed ** 2
            pct = runs ** 2 / denominator if denominator else None
            team["pythagorean"] = {
                "exponent": 2.0,
                "winPct": pct,
                "expectedWins": pct * games if pct is not None else None,
                "luck": number(team["record"].get("wins")) - pct * games if pct is not None else None,
            }

    all_players: dict[str, dict[str, Any]] = {}
    for period_key in ("2025", "2026", "combined"):
        for player in periods[period_key]["players"]:
            entry = all_players.setdefault(player["key"], {
                "key": player["key"], "name": player["name"], "team": player["team"],
                "headshot": player.get("headshot"), "positions": player.get("positions") or [],
                "currentRoster": player.get("currentRoster", False), "seasons": [],
            })
            if period_key in {"2025", "2026"} and (player.get("hitting") or player.get("pitching")):
                entry["seasons"].append(int(period_key))
            if player.get("currentRoster"):
                entry.update(team=player["team"], headshot=player.get("headshot") or entry.get("headshot"), positions=player.get("positions") or entry.get("positions"), currentRoster=True)
    for entry in all_players.values():
        if entry["currentRoster"] and 2026 not in entry["seasons"]:
            entry["seasons"].append(2026)
        entry["seasons"].sort()
    directory = sorted(
        (
            player
            for player in all_players.values()
            if player["seasons"] or player.get("currentRoster")
        ),
    ...
    result = {
        "meta": {
            ...
            "snapshot": periods["2026"]["meta"]["snapshot"],
            "generatedAt": datetime.now(timezone.utc).isoformat(),
            "sourceNote": periods["2026"]["meta"]["sourceNote"],
        },
        "directory": directory,
        "periods": periods,
        "leaderboards": leaderboards,
        "tto": tto,
        "pythagoreanStudy": pythagorean_study(periods),
    }
    SITE_DATA_DESTINATION.write_text(json.dumps(result, separators=(",", ":")), encoding="utf-8")
    return result
...
if __name__ == "__main__":
    result = build()
    print(
        f"Built Softball Savant with {len(result['directory'])} players, "
        f"{len(result['periods']['2026']['teams'])} teams, and "
        f"{result['tto']['meta']['plate_appearances']:,} TTO plate appearances."
    )
